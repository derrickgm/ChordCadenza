...
!ifdef X64
  ;64-bit settings
  !define BUILD_DIR "D:\D2\Dev\CS.Express\${VSNAME}\${VSNAME}\bin\Release X64"
  InstallDir "$PROGRAMFILES64\${PRODUCT_NAME}"
  !ifdef STARTERSOUNDFONT
    OutFile "${OUTDIR}\Setup ${PRODUCT_NAME}64font_${PRODUCT_VERSION}.exe"
  !else
    ;OutFile "D:\NSIS\Setup ${PRODUCT_NAME}64.exe"
    OutFile "${OUTDIR}\Setup ${PRODUCT_NAME}64_${PRODUCT_VERSION}.exe"
  !endif
!else
  ;32-bit settings
  !define BUILD_DIR "D:\D2\Dev\CS.Express\${VSNAME}\${VSNAME}\bin\Release"
  InstallDir "$PROGRAMFILES\${PRODUCT_NAME}"
  !ifdef STARTERSOUNDFONT
    OutFile "${OUTDIR}\Setup ${PRODUCT_NAME}32font_${PRODUCT_VERSION}.exe"
  !else
    OutFile "${OUTDIR}\Setup ${PRODUCT_NAME}32_${PRODUCT_VERSION}.exe"
  !endif
!endif
...
Section -Main 
  ...
  File "${BUILD_DIR}\${VSNAME}.exe"
  ;File "${BUILD_DIR}\${VSNAME}.exe.config"
  ;File "${BUILD_DIR}\${VSNAME}.exe.manifest"
  
  File "${BUILD_DIR}\Bass.dll"
  File "${BUILD_DIR}\Bass_fx.dll"
  File "${BUILD_DIR}\Bassasio.dll"
  File "${BUILD_DIR}\Bassmidi.dll"
  File "${BUILD_DIR}\Bassmix.dll"
  File "${BUILD_DIR}\Bass.Net.dll"
  
  SetOutPath "$APPDATA\${VSNAME}"
  File "${CFG_BUILD_DIR}\InitialScreen.dat"
  File "${CFG_BUILD_DIR}\Chords.dat"
  ;File "${CFG_BUILD_DIR}\${VSNAME}.ini"
  ;File "${CFG_BUILD_DIR}\FrmNMColours.ini"
  ;File "${CFG_BUILD_DIR}\FrmSCColours.ini"
  ;File "${CFG_BUILD_DIR}\Switch.ini"
  ;File "${CFG_BUILD_DIR}\ChordCfg.Ini"
...
Function .onInit
  ...
  ${IfNot} ${RunningX64}
    MessageBox MB_ICONINFORMATION|MB_OK "This is a 64-bit application, but you are not running a 64-bit system - you need to download the 32-bit version of this application."
	Abort
  ${EndIf}
